...
- 文件操作：复制 / 移动 / 重命名 / 删除
- 文档按标题拆分（一级、二级、一级+二级）
- 文档拆分导入格式可选（自动 / DOCX / Markdown / TXT / PDF）
- 文档拆分导出格式可选（原格式 / DOCX / Markdown / TXT / PDF）
- OCR 图片文字提取（可选）
- Dry Run（预演模式）
- 进度条、日志、JSON 报告
- Windows `exe` 与安装包脚本
...
- 输入 `md/txt/pdf`：
  ...
  - 输入 `pdf` 且导出 `pdf`：按页范围分片输出（默认自动保持 `pdf`）

### 2.3 关键限制（请注意）

- `md/txt -> docx` 属于“重建”，不可能保留原 Word 样式细节
- `docx -> md/txt` 属于“降级导出”，复杂排版会损失
- `docx/md/txt -> pdf` 当前未实现（仅支持 `pdf -> pdf`）
- `pdf` 标题识别基于文本模式启发式，复杂版式（多栏、扫描件）可能需要手工调整
...
- `src/fileops/operations.py`：复制/移动/重命名/删除
